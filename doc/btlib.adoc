...
Mark Willson
5.0.2, 03-05-2024: AsciiDoc documentation
:toc: left
...
`bt` commands have both full and abbreviated versions. The
...
Creates a data buffer called _bufname_. If a numeric _size_
is given, the buffer is created with that number of bytes. The
...
Deletes an existing data buffer identified by _bufname_.

`buffer-list,bl`::
Lists the names of the currently defined data buffers on
stdout.

`check-order,co` [s] [c] ::

Checks the lexicographic order of keys in the current root,
...
Creates a new B Tree file. If a file of the same name
...
Switches the current root to the root named _rootname_ in
...
Closes the in-use B Tree file. The next available open file, if
...
Defines a new key in the current root of the in-use B Tree
...
Prints, in a decoded form, the data segment address
...
Creates a new B Tree index root, named _rootname_ in the
...
Sets or unsets support for duplicate keys in the current
root. When _on_ is specified, duplicate keys are permitted. When _off_,
duplicate keys are not permitted .

`echo,ec` {on | off} ::

When `echo` is _on_, commands read from an link:#e[`execute`] file
...
When `error` is _on_, an execution error while reading commands
...
Causes commands to be read and executed from the file denoted
...
Attempts to locate _key_ in the current root of the in-use B Tree
...
Attempts to locate _key_ in the current root of the in-use B
...
Lists the set of open B Tree index files. To change the
current file, issue a link:#u[`use`] command.

`list,l` [c]::

Displays all key and associated value, starting from the current
...
Displays all keys, and associated data records, starting
...
Displays all keys, and associated data records,
...
Displays all keys, and associated value, prior to the
...
Displays all keys, but not associated value, starting
from the current key in the current root of the in-use B Tree index
file. The current key is set by the last link:#f[`find`],
link:#fd[`find-data`], link:#n[`next`], link:#nd[`next-data`],
link:#prv[`previous`] or link:#pd[`previous-data`] command.
...
Acquires exclusive access to the in-use B Tree file which was
originally opened in shared mode. If the file was opened in exclusive
mode (the default), `lock` will have no effect.

`next,n`::

Displays the key following the current key in the current root of
...
Displays the key and associated data record following the
current key in the current root of the in-use B Tree index file. The
current key is set by the last link:#f[`find`], link:#fd[`find-data`],
link:#n[`next`], link:#nd[`next-data`], link:#prv[`previous`] or
link:#pd[`previous-data`] command.
...
Opens the existing B Tree file identified by _filename_. If the
...
Opens the existing B Tree file identified by _filename_
...
Sets the position in the current root. _s_ will cause the
position to be set prior to the first key in the index, _e_ will cause
the position to be set after the last key.

`prompt,p`::

Toggles the display of a command prompt when `bt` is ready for
the next user command.

`previous,prv`::

Displays the key prior to the current key in the current root
...
Displays the key and associated data record prior to the
current key in the current root of the in-use B Tree index file. The
current key is set by the last link:#f[`find`], link:#fd[`find-data`],
link:#n[`next`], link:#nd[`next-data`], link:#prv[`previous`] or
link:#pd[`previous-data`] command.
...
Terminates `bt`. Any open B Tree files will be closed
automatically.

`remove,r` <key>::

Removes a previously defined key in the current root of the
in-use B Tree index file. The key name is specified by _key_.

`remove-cur,rc`::

Removes the current key in the current root of the
...
Removes an existing B Tree index root, named _rootname_ in
...
Displays B Tree debug information. The option specified is passed
...
Displays the number of bytes occupied by the data record
...
Passes the text following the system command to the shell for
execution.

`use,u` <filename>::

Changes the in-use B Tree file to _filename_. The file must have
already been opened or created, using the link:#o[`open`] or
link:#c[`create`] command.

`update-data,ud` <key> {<string> | <*bufname>} ::

Updates an existing key with a new associated data record
in the current root of the in-use B Tree index file. _key_ defines the
key name. Data can be provided in one of two ways: either a plain string
or the name of a previously defined buffer can be specified. If the
latter, it should be indicated by a leading _*_.
...
Updates the current key with a new associated data
record in the current root of the in-use B Tree index file. The current
key is set by the last link:#f[`find`], link:#fd[`find-data`],
link:#n[`next`], link:#nd[`next-data`], link:#prv[`previous`] or
...
Unlocks the in-use B Tree file, if it was locked with the `lk`
...
`update-value-current,uvc` <value>::

Modifies the value associated with current key _key_ in the current
root of the in-use B Tree index file. The current key is set by the
last link:#f[`find`], link:#fd[`find-data`], link:#n[`next`],
link:#nd[`next-data`], link:#prv[`previous`] or
link:#pd[`previous-data`] command.

`write-threshold,wt` <threshold>::

Sets the number of index block updates beyond which the block contents
must be writtent to disk. A _threshold_ of zero means writes will not
take place unless a block must be flushed to disk.

`help,?` [<cmd>] ::

Displays a list of `bt` commands and a terse description of syntax and
usage. If _cmd_ is specified, only help on that command will be
displayed.

`comment,#`::

Any line starting with the `comment` keyword or a
`#` will be ignored. Note that `bt` will also ignore blank lines.
...
| 5.0.2 | 3rd May, 2024 | Documentation converted to AsciiDoc and placed in source code tree
...
This manual was written in AsciiDoc using Emacs. Html is generated by
Asciidoctor.
